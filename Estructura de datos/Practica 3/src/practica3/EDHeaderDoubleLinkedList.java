...
public class EDHeaderDoubleLinkedList<T> implements List<T> {
	...
	private Node findNode(Object item) {
		if(this.size>0){
			...
			while(aux != this.header){							//Recorro hasta encontrar la cabecera
				if(compareNull(aux.data,item)) return aux;
				aux = aux.next;
			}
		}
		return null;
	...
	public void add(int index, T element) {
		Node n = findIndex(index); //Busco el nodo a partir del indice
		insertBefore(n,element);
	}

	@Override
	public boolean remove(Object item) {
		Node n = findNode(item); 	//Busco el nodo
		if(n != null) {
			removeNode(n);
			return true;
		}
	...
	public int lastIndexOf(Object item) {
		...
		int index=0;
		while(aux!=header){						//Recorro hasta encontrar el Nodo cabecera
			if(compareNull(aux.data,item))
				return size-index-1;
			aux=aux.prev;
			index++;
