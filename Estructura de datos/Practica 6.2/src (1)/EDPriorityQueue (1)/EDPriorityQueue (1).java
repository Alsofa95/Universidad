...
public class EDPriorityQueue<E>  {
	...
	private void sink (int parent) {
		int h_izq = 2*parent+1;
		int h_der = 2*parent+2;
		//TENGO QUE COMPROBAR CUAL ES MAYOR DE LOS DOS

		if(data[h_izq] != null && this.compare(data[parent],data[h_izq])>1){
			swap(parent,h_izq);
		}
		else
			if(data[h_der] != null && this.compare(data[parent],data[h_der])>1){
				swap(parent,h_der);
			}
